...
```bash
StackName="vpc-network-tester"

TIER="dev|int|stage|prod"  ### 👈🏾👈🏾 PICK on value !!!!!!!
AWSPROFILE=.. ..
AWSREGION=.. ..

SubnetId=".. .."  #### qa|test-tier:   subnet-0ddf67617142d837c subnet-0e943b5183090101c
SecurityGroupId=".. .."  ### qa|test-tier:  sg-0afea5dd76df643b6


\rm -rf .aws-sam/   ~/.aws-sam/   ~/node_modules/  ./src/node_modules/
(cd src/vpc-network-tester;           \rm -rf package-lock.json;   npm install --include-dev)
(cd src/VPCEndPt-SecretsMgr-tester;   \rm -rf package-lock.json;   npm install --include-dev)
sam build

sam deploy --stack-name "${StackName}" --capabilities CAPABILITY_IAM                \
    --parameter-overrides     \
        Tier=${TIER}              DateTimeStamp=$(date +"%Y-%M-%dT%H:%M:%S")        \
...
```bash
sam delete --stack-name network-tester
```
