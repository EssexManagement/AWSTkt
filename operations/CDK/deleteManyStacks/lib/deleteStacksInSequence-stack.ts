...
export class DeleteStacksInSequence extends cdk.Stack {
constructor(scope: Construct, id: string,
    ...
    const thisStepFuncNAME   = "DeleteStacksInSequence"
    const secondOtherSfnNAME = "DeleteStacksInParallel"
    // const thisStepFuncNAME = constants.get_FULL_AWS_RESOURCE_PREFIX(tier, git_branch, "sfn-cleanup-FAILED-stacks", constants.CDK_OPERATIONS_COMPONENT_NAME )
    ...
    const cfnParam_StepFuncToDeleteStacksInParallel = new cdk.CfnParameter(this, 'StepFuncToDeleteStacksInParallel', {
        type: 'String',
    ...
    const whetherJsonSequenceIsPresent = new sfn.Choice(this, "Is Input a Sequence-of-Stacks to be deleted?",
        { comment: "Within Input, '$.deleteInSequence' is present" }
    );
    ...
    //// --------------------------------------------------
    whetherJsonSequenceIsPresent.when( sfn.Condition.isPresent("$.deleteInSequence"), initializeLoop )
    ...
    ifDeleteStackInProgress.when(
        sfn.Condition.or(
            // sfn.Condition.stringEquals("$.StackDetails.StackDetails.StackStatus", "CREATE_COMPLETE"),
            // sfn.Condition.stringEquals("$.StackDetails.StackDetails.StackStatus", "CREATE_IN_PROGRESS"),
    ...
    ifDeleteStackInProgress.when(
        sfn.Condition.or(
          sfn.Condition.stringEquals("$.StackDetails.StackDetails.StackStatus", "ROLLBACK_COMPLETE"),
          sfn.Condition.stringEquals("$.StackDetails.StackDetails.StackStatus", "UPDATE_ROLLBACK_COMPLETE"),
          sfn.Condition.stringEquals("$.StackDetails.StackDetails.StackStatus", "ROLLBACK_COMPLETE"),
          sfn.Condition.stringEquals("$.StackDetails.StackDetails.StackStatus", "DELETE_COMPLETE"),
        ),
        INcrementCounter,
        { comment: "Done! Deleted", }
    )
    ifDeleteStackInProgress.otherwise( taskSNSTopicAborted )
