...
def fetch_attachments(downloader, pdfs):
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=min(len(pdfs), 6)) as executor:
...
def lambda_handler(event, context):
    ...
    attachments = [a for a in event.get("attachments", []) if urlparse(a).path.lower().endswith(".pdf")]
