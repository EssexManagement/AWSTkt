...
class GetHashesForLambdaLayers(GenericAWSCLIScript):
    ...
    def __init__(self,
        ...
        app_specific_lambda_layers      :list[str] = []
        ...
        for lyr in layers_list:
            ...
            if regex.match( pattern=lambdalayer_regex, string=lyr_nm ):
                ...
                if lyr_nm in app_specific_lambda_layer_names:
                    print(lyr_nm +' : '+ lyr_ver_arn, end=".. ", flush=True)
        ...
        with open(file_to_save_hashes_into, 'w') as f:
            ...
            f.write(  "lambda_layer_hashes = " )
            f.write(json.dumps({
                name: arn for name, arn in zip(app_specific_lambda_layer_names, app_specific_lambda_layer_arns)
            }, indent=4))
            f.write( '\n\n' + w1 + w2 + w1 + w2 )
            f.close()
